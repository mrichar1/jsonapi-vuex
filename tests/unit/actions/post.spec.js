...
describe("post", function() {
  ...
  it("should make an api call to POST item(s)", async function() {
    ...
    expect(this.mock_api.history.post[0].url).to.equal(`/${norm_widget_1['_jv']['type']}`)
  })
  ...
  it("should accept axios config as the 2nd arg in a list", async function() {
    ...
    await jsonapiModule.actions.post(stub_context, [ norm_widget_1, { params: params } ])
  ...
  it("should POST data", async function() {
    ...
    expect(JSON.parse(this.mock_api.history.post[0].data)).to.deep.equal({ data: json_widget_1 })
  })
